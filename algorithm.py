...
class Sample(object):
    ...
    def state_to_feats(self, nbuffer_feats: int = 2, nstack_feats: int = 2):
        ...
        stack_feats = ['<PAD>', '<PAD>'] * nstack_feats 
        buffer_feats = ['<PAD>', '<PAD>'] * nbuffer_feats 

        for i in range(min(nstack_feats, len(self.state.S))):
            token = self.state.S[-(i+1)]
            stack_feats[i*2] = token.form
            stack_feats[i*2+1] = token.upos

        for i in range(min(nbuffer_feats, len(self.state.B))):
            token = self.state.B[i]
            buffer_feats[i*2] = token.form
            buffer_feats[i*2+1] = token.upos

        # Concatenamos las palabras del stack con las del buffer y a su vez con las etiquetas correspondientes
        word_features = stack_feats[::2] + buffer_feats[::2]
        upos_features = stack_feats[1::2] + buffer_feats[1::2]
        
        return word_features + upos_features
...
class ArcEager():
    ...
    def LA_is_valid(self, state: State) -> bool:
        ...
        # LA es válido cuando haya por lo menos más de un token en el Stack 
        # y no se ha creado aún un arco donde la cima del stack (i) es el nodo hijo

        # Se necesitan por lo menos dos tokens en el Stack, 
        # uno que actúe como head y otro que reciba el arco
        if len(state.S) > 1: 
            topS= state.S[-1]
            for arc in state.A:        
                if arc[2] == topS.id:  
    ...
    def LA_is_correct(self, state: State) -> bool:
        ...
        """
        return state.S[-1].head == state.B[0].id
    ...
    def RA_is_valid(self, state: State) -> bool:
        ...
        """
        # RA es válido cuando hay como mínimo un token en el Stack y otro en el Buffer
        # y no se ha creado aún un arco donde el frente del buffer es el nodo hijo de un arco
    ...
    def RA_is_correct(self, state: State) -> bool:
        ...
        """
        return state.B[0].head == state.S[-1].id 
    ...
    def REDUCE_is_valid(self, state: State) -> bool:
        ...
        """
        # Reduce es válido cuando hay por lo menos un token en el Stack y 
    ...
    def REDUCE_is_correct(self, state: State) -> bool:
        ...
        for word in state.B:
            if word.head == topS.id:
                return False
        return True


    def has_head(self, s, Arc) -> bool:
        for arc in Arc:
            if arc[2] == s.id:
                return True  
        return False 
